...
def load_variable_help() -> Dict[str, str]:
    ...
    if not meta_path.exists():
        ...
        default_help = {
            ...
            "Emotional problem": "Emotional problem (0=No, 1=Yes)",
            "Memory-related disease": "Memory-related disease (0=No, 1=Yes)",
            "Arthritis or rheumatism": "Arthritis or rheumatism (0=No, 1=Yes)",
            "Falldown": "Falldown (0=No, 1=Yes)",
            ...
            "Inpatient times": "Inpatient times (≥0)",
            "Online participation": "Online participation (0=No, 1=Uses internet)",
            "Intellectual activities": "Intellectual activities (0=No, 1=Participates)"
        }
        return default_help
...
def build_numeric_scaler(x_train_scaled: pd.DataFrame) -> Optional[StandardScaler]:
    """Create a scaler for Age and Inpatient times.
    ...
    if generic_path.exists():
        ...
        inp_col = normalized.get("inpatienttimes") or normalized.get("inpatient_time")
        selected = []
        if age_col:
            selected.append(age_col)
        if inp_col:
            selected.append(inp_col)
        if len(selected) == 2:
            scaler = StandardScaler().fit(raw_df[[age_col, inp_col]].rename(columns={age_col: "Age", inp_col: "Inpatient times"}))
            return scaler
    ...
    if set(target_cols).issubset(z.columns):
        ...
        try:
            ...
            inp_sigma = 5.0 / max(z_span[1], 1e-6)   # assume 0..5 typical span
            sigma = np.array([age_sigma, inp_sigma])
...
def render_title():
    st.set_page_config(
        page_title="Trunk Pain ADL Disability Risk Predictor",
        page_icon="🩺",
        layout="wide",
        initial_sidebar_state="collapsed",
    )
    st.title("Trunk Pain ADL Disability Risk Predictor")
    st.caption(
...
def main():
    ...
    try:
        ...
        with st.form("input_form"):
            ...
            marital = sb(2, "Marital status", {"Unmarried (0)": 0, "Married (1)": 1}, "Married (1)")
            edu = sb(0, "Education level", {"Below high school (1)": 1, "High school and above (2)": 2}, "Below high school (1)")
            htn = sb(1, "Hypertension", {"No (0)": 0, "Yes (1)": 1}, "No (0)")
            db = sb(2, "Diabetes", {"No (0)": 0, "Yes (1)": 1}, "No (0)")
            stroke = sb(0, "Stroke", {"No (0)": 0, "Yes (1)": 1}, "No (0)")
            emo = sb(1, "Emotional problem", {"No (0)": 0, "Yes (1)": 1}, "No (0)")
            mem = sb(2, "Memory-related disease", {"No (0)": 0, "Yes (1)": 1}, "No (0)")
            arth = sb(0, "Arthritis or rheumatism", {"No (0)": 0, "Yes (1)": 1}, "No (0)")
            fall = sb(1, "Falldown", {"No (0)": 0, "Yes (1)": 1}, "No (0)")
            ...
            inpatient_raw = cols[2].number_input(
                "Inpatient times",
                min_value=0,
                max_value=20,
                value=0,
                step=1,
                help=variable_help.get("Inpatient times", "(≥0)"),
            )
            online = sb(0, "Online participation", {"No (0)": 0, "Uses internet (1)": 1}, "No (0)")
            intel = sb(
                1,
                "Intellectual activities",
                {"No (0)": 0, "Participates (1)": 1},
                "No (0)",
            )
            ...
            with st.expander("📋 Variable Definitions"):
                st.markdown("""
                **physical activity level**: MET-minutes/week = MET value × days × duration  
                **Intellectual activities**: Engaged in mahjong, chess, card games, or attended community clubs and educational courses.  
                **Inpatient times**: How many times have you received inpatient care during the past year?
                """)
        ...
        numerics = [c for c in ["Age", "Inpatient times"] if c in sample_df_unscaled.columns]
        ...
        if numerics and scaler is not None:
            ...
            st.warning(
                "Automatic standardization for Age and Inpatient times is not available. "
        ...
        if submitted:
            ...
            with st.spinner("Calculating SHAP explanation..."):
                ...
                background = x_train_scaled.sample(
                    n=min(100, len(x_train_scaled)), random_state=42
                )
        ...
        with st.expander("📖 Usage Instructions"):
            st.markdown("""
            **Model Description:**
            - This model uses machine learning algorithms to predict ADL (Activities of Daily Living) disability risk in patients with trunk pain
            ...
            **Notes:**
            - Age and inpatient times are automatically standardized
